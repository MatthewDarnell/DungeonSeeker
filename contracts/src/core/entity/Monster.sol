...
contract Monster is IEntity {
    ...
    function setStats(uint AC, uint INT, uint WIS, uint DEX, uint STL, uint PER, uint ARC, uint NAT, uint SUR) public {
        //require(msg.sender == gm, "Only the GM can Set Player Stats!");
        uint _stats = packStats(AC, INT, WIS, DEX, STL, PER, ARC, NAT, SUR);
        stats = _stats;
    }
}